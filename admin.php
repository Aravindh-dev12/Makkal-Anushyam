...
<body class="bg-slate-50 text-slate-800 antialiased min-h-screen flex flex-col">
    ...
    <script>
        ...
        function peakPowerCacheKey(unit) {
            return 'admin_peak_kw_' + localDateKey() + '_' + unit;
        }

        function peakHourCacheKey(unit) {
            return 'admin_peak_hour_' + localDateKey() + '_' + unit;
        }
        ...
        function loadCachedPeak(unit) {
            const cached = parseFloat(localStorage.getItem(peakPowerCacheKey(unit)) || '0');
            return Number.isFinite(cached) && cached > 0 ? cached : 0;
        }

        function loadCachedPeakHour(unit) {
            return localStorage.getItem(peakHourCacheKey(unit)) || '--:--';
        }

        function recordPeakPower(unit, kw, timestamp = new Date()) {
            const st = plantState[unit];
            if (!st || kw <= 0) return;
            if (kw > (st.peakInverterKw || 0)) {
                st.peakInverterKw = kw;
                const d = timestamp instanceof Date ? timestamp : new Date(timestamp);
                st.peakHour = String(d.getHours()).padStart(2, '0') + ':' +
                    String(Math.floor(d.getMinutes() / 5) * 5).padStart(2, '0');
                localStorage.setItem(peakPowerCacheKey(unit), String(st.peakInverterKw));
                localStorage.setItem(peakHourCacheKey(unit), st.peakHour);
            }
        }
        ...
        function updateDailyEnergy(unit) {
            ...
            const inverterTotal = Object.values(st.inverters || {})
                .filter((_, k) => !String(k).toLowerCase().includes('vcb'))
                .reduce((sum, inverter) => sum + (Number(inverter.dailyGen) || 0), 0);
            const vcbVal = Number(st.vcbToday) > 0 ? Number(st.vcbToday) : 0;
            const bestToday = Math.max(vcbVal, inverterTotal, loadCachedToday(unit), Number(st.dailyEnergy) || 0);
        ...
        plants.forEach(p => {
            plantState[p.id] = {
                ...
                inverters: {},
                peakInverterKw: loadCachedPeak(p.id),
                peakHour: loadCachedPeakHour(p.id),
        ...
        function renderCards() {
                    ...
                    <div id="card-${p.id}" class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm flex flex-col h-[560px] transition-all hover:-translate-y-1 hover:shadow-lg duration-200">
                        ...
                        <div class="p-4 grid grid-cols-2 gap-3 shrink-0">
                            ...
                            <div class="col-span-2 bg-amber-50/60 rounded-lg p-3 border border-amber-100 flex items-center justify-between">
                                <div>
                                    <div class="text-amber-600 text-[10px] font-bold uppercase mb-1 tracking-wider"><i class="fa-solid fa-chart-line mr-1"></i>Today Peak &bull; Combined Inverters</div>
                        ...
                        <div class="px-4 pb-4 flex flex-col overflow-hidden flex-grow">
            ...
            `).join('');
            plants.forEach(p => updateUI(p.id));
        }
        ...
        function updateUI(unit_id) {
            ...
            for (const inv in st.inverters) {
                if (inv.toLowerCase().includes('vcb') || inv.toLowerCase().includes('transformer')) continue;
                sumInverterPower += (st.inverters[inv].power || 0);
            }

            const finalActivePower = (st.hasVCB && st.vcbPower > 0) ? st.vcbPower : sumInverterPower;
            const activeLabel = (st.hasVCB && st.vcbPower > 0) ? 'VCB Active' : 'Active Power';
            ...
            const sortedInverters = Object.keys(st.inverters)
                .filter(invName => !invName.toLowerCase().includes('vcb') && !invName.toLowerCase().includes('transformer'))
                .sort((a, b) => {
            ...
            if (finalActivePower > 0) {
                ...
                badge.innerHTML = `<span class="h-2 w-2 rounded-full bg-emerald-500 inline-block mr-1 shadow-[0_0_5px_green]"></span> ACTIVE`;
                ...
                badge.innerHTML = `<span class="h-2 w-2 rounded-full bg-slate-400 inline-block mr-1"></span> STANDBY`;
                card.classList.remove('alert-state');
            }
        }
        ...
        function updateOverallActivePower() {
            const totalKw = Object.values(plantState).reduce((total, state) => {
                const inverterKw = Object.values(state.inverters || {})
                    .filter((_, k) => !String(k).toLowerCase().includes('vcb'))
                    .reduce((sum, inverter) => sum + (Number(inverter.power) || 0), 0);
                const active = (state.hasVCB && Number(state.vcbPower) > 0) ? Number(state.vcbPower) : inverterKw;
                return total + (active > 0 ? active : 0);
            }, 0);
        ...
        function loadInverterPeakHistory(unit, data, requestedDevice = '') {
            ...
            if (historicalPeakKw > (st.peakInverterKw || 0)) {
                st.peakInverterKw = historicalPeakKw;
                st.peakHour = historicalPeakTime;
                localStorage.setItem(peakPowerCacheKey(unit), String(st.peakInverterKw));
                localStorage.setItem(peakHourCacheKey(unit), st.peakHour);
            }
            if (st.liveCombinedKw > (st.peakInverterKw || 0)) {
                recordPeakPower(unit, st.liveCombinedKw);
            }
            updateUI(unit);
        }
                    ...
                    if (data.values) {
                        ...
                        if (!isVCBMsg) {
                            ...
                            for (const pk in data.values) {
                                const pkl = pk.toLowerCase();
                                if (/active.*power|ac.*power|power.*ac|a\.c\..*power/i.test(pkl) && !/reactive|apparent|3.phase|limit|ratio/i.test(pkl)) {
                            ...
                            if (liveCombinedPower > (plantState[unit].peakInverterKw || 0)) {
                                recordPeakPower(unit, liveCombinedPower);
                            }
                            }
                        }
                    }
