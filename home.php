...
<body class="h-full bg-slate-50 text-slate-800 font-sans">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                        ...
                        <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 shadow-xs hover:shadow-md transition">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <div class="w-8 h-8 rounded-lg bg-rose-100 text-rose-600 flex items-center justify-center">
                                    <i class="fa-solid fa-temperature-high text-sm"></i>
                                </div>
                            </div>
                            <div class="flex items-baseline gap-1 mt-1">
                                <span class="font-black text-rose-700 text-2xl sm:text-3xl font-mono" id="wmos_ptemp">--</span>
                                <span class="text-sm font-bold text-rose-500">°C</span>
                            </div>
                        ...
                        <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 shadow-xs hover:shadow-md transition">
                            ...
                            <div class="flex items-baseline gap-1 mt-1">
                                <span class="font-black text-amber-600 text-2xl sm:text-3xl font-mono" id="wmos_atemp">--</span>
                                <span class="text-sm font-bold text-amber-500">°C</span>
                            </div>
                        ...
                        <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 shadow-xs hover:shadow-md transition">
                            ...
                            <div class="flex items-baseline gap-1 mt-1">
                                <span class="font-black text-sky-600 text-2xl sm:text-3xl font-mono" id="wmos_wind">--</span>
                                <span class="text-sm font-bold text-sky-500">m/s</span>
                            </div>
                        ...
                        <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 shadow-xs hover:shadow-md transition">
                            ...
                            <div class="flex items-baseline gap-1 mt-1">
                                <span class="font-black text-indigo-600 text-2xl sm:text-3xl font-mono" id="wmos_hum">--</span>
    ...
    <script>
        ...
        const peakCacheKey = 'vs_peak_' + currentPlant + '_' + localDateKey();
        const cachedPeakPower = parseFloat(localStorage.getItem(peakCacheKey) || '0') || 0;
        const state = { vcbPower: 0, dailyEnergy: 0, inverters: {}, peakPower: cachedPeakPower, isRunning: false, runSeconds: 0 };
        if (cachedPeakPower > 0) {
            const el = document.getElementById('vcb_peak');
            if (el) el.textContent = cachedPeakPower.toFixed(2);
        }
        ...
        const todayEnergyCacheKey = 'vs_today_energy_' + currentPlant + '_' + localDateKey();
        let energyFloor = parseFloat(localStorage.getItem(todayEnergyCacheKey) || '0') || 0;
        state.dailyEnergy = energyFloor;
        if (energyFloor > 0) {
            const etodayEl = document.getElementById('vcb_etoday');
            if (etodayEl) etodayEl.innerHTML = energyFloor.toFixed(2) + ' <span class="text-sm font-bold text-purple-600">kWh</span>';
        }

        function calculateTodayEnergy(incomingVcbToday = null) {
            const invTotal = Object.values(state.inverters || {}).reduce((sum, inv) => sum + (Number(inv.dailyGen) || 0), 0);
            let vcbVal = 0;
            if (incomingVcbToday !== null && incomingVcbToday > 0) {
                vcbVal = incomingVcbToday;
                state.rawVcbToday = incomingVcbToday;
            } else if (state.rawVcbToday > 0) {
                vcbVal = state.rawVcbToday;
            }
            // NEVER decrease — always take the max of all sources including previous value
            const candidate = Math.max(vcbVal, invTotal);
            if (candidate > energyFloor) {
                energyFloor = candidate;
                state.dailyEnergy = candidate;
                localStorage.setItem(todayEnergyCacheKey, String(candidate));
            } else {
                // Keep the floor — never drop
                state.dailyEnergy = energyFloor;
            }
            return state.dailyEnergy;
        }
        ...
        function updateDash() {
            ...
            if (final > state.peakPower) {
                state.peakPower = final;
                localStorage.setItem(peakCacheKey, state.peakPower.toFixed(2));
            }
        ...
        function connectWS() {
            ...
            ws.onopen = function() {
                ...
                if (currentPlant !== 'vinoba-velliyanai') {
            ...
            ws.onmessage = function(e) {
                try {
                    ...
                    if (isWeather && d.values) {
                        // Radiation
                        let radVal = d.values["raw data"] !== undefined ? d.values["raw data"] :
                                     (d.values["temp data"] !== undefined && deviceStr.includes('pyran') ? d.values["raw data"] : undefined);
                        if (radVal === undefined) {
                            for (const k in d.values) {
                                if (/rad|irradiance|pyran/i.test(k)) { radVal = d.values[k]; break; }
                            }
                        }
                        ...
                        // Ambient Temperature
                        let atempVal = d.values["Ambient temperature"] !== undefined ? d.values["Ambient temperature"] :
                                       (d.values["ambient temperature"] !== undefined ? d.values["ambient temperature"] : undefined);
                        if (atempVal === undefined && deviceStr.includes('ambient')) {
                            for (const k in d.values) {
                                if (/temp/i.test(k)) { atempVal = d.values[k]; break; }
                            }
                        }
                        ...
                        // Wind Speed
                        let windVal = d.values["windspeed"] !== undefined ? d.values["windspeed"] :
                                      (d.values["wind speed"] ?? d.values["Wind Speed"]);
                        if (windVal === undefined && deviceStr.includes('wind')) {
                            for (const k in d.values) {
                                if (/wind|speed/i.test(k)) { windVal = d.values[k]; break; }
                            }
                        }
                        if (windVal !== undefined && windVal !== null) {
                            const wNum = parseFloat(windVal);
                            const el = document.getElementById('wmos_wind');
                            if (el) el.textContent = !isNaN(wNum) ? wNum.toFixed(1) : windVal;
                        }

                        // Humidity
                        let humVal = d.values["humidity"] !== undefined ? d.values["humidity"] : d.values["Humidity"];
                        if (humVal === undefined && deviceStr.includes('humid')) {
                            for (const k in d.values) {
                                if (/hum/i.test(k)) { humVal = d.values[k]; break; }
                            }
                        }
                        if (humVal !== undefined && humVal !== null) {
                            const hNum = parseFloat(humVal);
                            const el = document.getElementById('wmos_hum');
                            if (el) el.textContent = !isNaN(hNum) ? hNum.toFixed(1) : humVal;
                        }
                        return; // Weather handled
                    }
                    ...
                    if (d.virtualTags && d.virtualTags["vcb-today"] !== undefined) {
                        const rawToday = parseFloat(d.virtualTags["vcb-today"].value) || 0;
                        if (rawToday > 0) state.rawVcbToday = rawToday;
                        calculateTodayEnergy(rawToday > 0 ? rawToday : null);
                    }

                    // Detect inverter message: has power or has numbered current keys
                    const hasInvPower = keys.some(pk => {
                        const pkl = pk.toLowerCase();
                        if (/limit|ratio/i.test(pkl)) return false;
                    ...
                    if (!isVcbMessage && !isTransformerMessage && (hasInvPower || hasNumberedCurrents || taskStr === 'inverter')) {
                        ...
                        let totalGen = 0, dailyGen = 0;
                        for (const gk in d.values) {
                            const gkl = gk.toLowerCase();
                            if (/total.*gen/i.test(gkl)) totalGen = parseFloat(d.values[gk]) || 0;
                            if (/daily.*gen|today.*gen/i.test(gkl)) dailyGen = parseFloat(d.values[gk]) || 0;
                        }
                        ...
                        for (const pk in d.values) {
                            const pkl = pk.toLowerCase();
                            if (/limit|ratio/i.test(pkl)) continue;
